...
// ==========================================================================
// ⚡ Universal Command Palette (Ctrl + K) & Favorites System
// ==========================================================================
(function initCommandPaletteSystem() {
  let selectedIndex = 0;
  let currentResults = [];

  window.openCommandPalette = function() {
    let backdrop = document.getElementById('cmdPaletteBackdrop');
    if (!backdrop) {
      injectCommandPaletteDOM();
      backdrop = document.getElementById('cmdPaletteBackdrop');
    }
  ...
  window.closeCommandPalette = function(e) {
    if (e && e.target && e.target.closest('.cmd-palette-modal') && !e.target.closest('.cmd-item') && !e.target.closest('.cmd-close-btn')) {
      return;
    }
  ...
  window.toggleFavoriteTool = function(toolId, event) {
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }
    let favs = JSON.parse(localStorage.getItem('aibots_favorite_tools') || '[]');
    if (favs.includes(toolId)) {
      favs = favs.filter(id => id !== toolId);
      if (typeof showToast === 'function') showToast('Removed from pinned favorites', 'info');
    } else {
      favs.push(toolId);
      if (typeof showToast === 'function') showToast('⭐ Added to pinned favorites!', 'success');
    }
    localStorage.setItem('aibots_favorite_tools', JSON.stringify(favs));
    
    const input = document.getElementById('cmdSearchInput');
    renderCommandResults(input ? input.value : '');
    window.renderFavoritesBar();
  };

  window.recordRecentTool = function(toolId) {
    if (!toolId) return;
    let recents = JSON.parse(localStorage.getItem('aibots_recent_tools') || '[]');
    recents = [toolId, ...recents.filter(id => id !== toolId)].slice(0, 6);
    localStorage.setItem('aibots_recent_tools', JSON.stringify(recents));
  };
  ...
  function injectCommandPaletteDOM() {
    ...
    div.id = 'cmdPaletteBackdrop';
    div.className = 'cmd-palette-backdrop';
    div.onclick = (e) => window.closeCommandPalette(e);
    div.innerHTML = `
      <div class="cmd-palette-modal" onclick="event.stopPropagation()">
        <div class="cmd-search-header">
          <i class="fas fa-magnifying-glass cmd-search-icon"></i>
          <input type="text" id="cmdSearchInput" class="cmd-search-input" placeholder="Search 24+ creative tools, commands, or math..." autocomplete="off" spellcheck="false">
          <span class="cmd-kbd-badge">ESC</span>
        </div>
        <div id="cmdResultsList" class="cmd-results-container"></div>
        <div class="cmd-footer">
          <div class="cmd-footer-keys">
            <span><kbd class="cmd-kbd-badge">↑</kbd> <kbd class="cmd-kbd-badge">↓</kbd> navigate</span>
            <span><kbd class="cmd-kbd-badge">↵</kbd> open</span>
            <span><kbd class="cmd-kbd-badge">ESC</kbd> close</span>
          </div>
          <span>AI Bots Studio &bull; 100% Private</span>
        </div>
      </div>
    `;
  ...
  function renderCommandResults(query) {
    ...
    const favs = JSON.parse(localStorage.getItem('aibots_favorite_tools') || '[]');
    const recents = JSON.parse(localStorage.getItem('aibots_recent_tools') || '[]');

    let html = '';
    currentResults = [];

    // Math Evaluation Shortcut (e.g. 50*20, sqrt(144), 100/4)
    if (q && /^[\d\s\+\-\*\/\^\(\)\.\%sqrtsinco]+$/.test(q) && /[0-9]/.test(q)) {
      try {
        const sanitized = q.replace(/sqrt/g, 'Math.sqrt');
        const mathRes = Function('"use strict";return (' + sanitized + ')')();
        if (typeof mathRes === 'number' && !isNaN(mathRes)) {
          html += `
            <div class="cmd-group-label"><span>⚡ Quick Math Result</span></div>
            <div class="cmd-item" onclick="navigator.clipboard.writeText('${mathRes}'); if(typeof showToast==='function') showToast('Copied ${mathRes} to clipboard!', 'success'); window.closeCommandPalette();">
              <div class="cmd-item-left">
                <div class="cmd-item-icon" style="background: rgba(59, 130, 246, 0.15); color: #3b82f6;">
                  <i class="fas fa-calculator"></i>
                </div>
                <div class="cmd-item-info">
                  <div class="cmd-item-title">${q} = <strong>${mathRes}</strong></div>
                  <div class="cmd-item-desc">Click to copy result &bull; Press Enter</div>
                </div>
              </div>
              <div class="cmd-item-right">
                <span class="cmd-kbd-badge">Copy Result</span>
              </div>
            </div>
          `;
        }
    ...
    if (!q) {
      // 1. Show Pinned Favorites
      if (favs.length > 0) {
        const favTools = AI_BOTS_TOOLS.filter(t => favs.includes(t.id));
        if (favTools.length > 0) {
          html += `<div class="cmd-group-label"><span>⭐ Pinned Favorites</span></div>`;
          favTools.forEach(tool => {
            html += renderToolItemHtml(tool, true);
            currentResults.push(tool);
          });
        }
      }

      // 2. Show Recently Used
      if (recents.length > 0) {
        const recentTools = recents.map(id => AI_BOTS_TOOLS.find(t => t.id === id)).filter(Boolean);
        if (recentTools.length > 0) {
          html += `<div class="cmd-group-label"><span>🕒 Recently Used</span></div>`;
          recentTools.forEach(tool => {
            html += renderToolItemHtml(tool, favs.includes(tool.id));
            currentResults.push(tool);
          });
        }
      }

      // 3. All Creative Tools
      html += `<div class="cmd-group-label"><span>🧰 All Studio Tools</span></div>`;
      AI_BOTS_TOOLS.forEach(tool => {
        html += renderToolItemHtml(tool, favs.includes(tool.id));
        currentResults.push(tool);
      });
      ...
      const matches = AI_BOTS_TOOLS.filter(t => {
      ...
      if (matches.length > 0) {
        html += `<div class="cmd-group-label"><span>Found ${matches.length} Tool${matches.length > 1 ? 's' : ''}</span></div>`;
        ...
        html += `
          <div style="padding: 36px 20px; text-align: center; color: var(--text-muted);">
            <i class="fas fa-magnifying-glass" style="font-size: 2rem; margin-bottom: 10px; opacity: 0.5; display: block;"></i>
            <div style="font-weight: 700; color: var(--text-primary); margin-bottom: 4px;">No tools found for "${query}"</div>
            <div style="font-size: 0.82rem;">Try searching for <em>video, pdf, image, background, qr</em></div>
          </div>
        `;
      }
    }
  ...
  function renderToolItemHtml(tool, isFav) {
    const isExternal = tool.url.startsWith('http');
    return `
      <a href="${tool.url}" class="cmd-item" onclick="window.recordRecentTool('${tool.id}'); if(typeof recordToolLaunchClick==='function') recordToolLaunchClick('${tool.id}');">
        <div class="cmd-item-left">
          <div class="cmd-item-icon" style="background: color-mix(in srgb, ${tool.color || 'var(--primary)'} 14%, transparent); color: ${tool.color || 'var(--primary)'};">
            <i class="fas ${tool.icon || 'fa-wrench'}"></i>
          </div>
          <div class="cmd-item-info">
            <div class="cmd-item-title">${tool.title} ${tool.badge ? `<span style="font-size: 0.68rem; font-weight: 800; padding: 2px 6px; border-radius: 6px; background: rgba(99,102,241,0.12); color: var(--primary); margin-left: 6px;">${tool.badge}</span>` : ''}</div>
            <div class="cmd-item-desc">${tool.description}</div>
          </div>
        </div>
        <div class="cmd-item-right">
          <button type="button" class="cmd-star-btn ${isFav ? 'starred' : ''}" onclick="window.toggleFavoriteTool('${tool.id}', event)" title="${isFav ? 'Unpin Favorite' : 'Pin to Favorites'}">
            <i class="${isFav ? 'fas' : 'far'} fa-star"></i>
          </button>
          <span class="cmd-kbd-badge">${isExternal ? '↗ Ext' : '↵ Open'}</span>
        </div>
      </a>
    `;
  }
  ...
  window.addEventListener('keydown', (e) => {
    ...
    // Slash '/' trigger when not in an input
  ...
  // Inject Nav Search Pill across all page headers
  function injectNavSearchPill() {
    const navActions = document.querySelector('.nav-actions');
    if (navActions && !navActions.querySelector('.nav-search-pill')) {
      const pill = document.createElement('button');
      pill.type = 'button';
      pill.className = 'nav-search-pill';
      pill.title = 'Search All Tools (Ctrl + K)';
      pill.onclick = () => window.openCommandPalette();
      pill.innerHTML = `
        <i class="fas fa-magnifying-glass"></i>
        <span>Search</span>
        <span class="cmd-kbd-badge">Ctrl K</span>
      `;
      navActions.insertBefore(pill, navActions.firstChild);
    }
  }

  // Homepage Favorites Bar Renderer
  window.renderFavoritesBar = function() {
    const bar = document.getElementById('favoritesBarContainer');
    if (!bar) return;
    ...
    let html = `
      <button type="button" class="fav-pill" style="background: linear-gradient(135deg, var(--primary), var(--secondary)); color: white; border: none;" onclick="window.openCommandPalette()">
        <i class="fas fa-bolt"></i> Spotlight (Ctrl+K)
      </button>
    `;

    if (favs.length > 0) {
      favs.forEach(id => {
        const tool = AI_BOTS_TOOLS.find(t => t.id === id);
        if (tool) {
          html += `
            <a href="${tool.url}" class="fav-pill" onclick="if(typeof recordToolLaunchClick==='function') recordToolLaunchClick('${tool.id}');">
              <span style="color: #f59e0b;">⭐</span> <i class="fas ${tool.icon}"></i> ${tool.title}
            </a>
          `;
        }
      });
    ...
    if (recents.length > 0) {
      const unstarredRecents = recents.filter(id => !favs.includes(id)).slice(0, 3);
      unstarredRecents.forEach(id => {
        const tool = AI_BOTS_TOOLS.find(t => t.id === id);
        if (tool) {
          html += `
            <a href="${tool.url}" class="fav-pill" style="opacity: 0.88;" onclick="if(typeof recordToolLaunchClick==='function') recordToolLaunchClick('${tool.id}');">
              <i class="fas fa-clock-rotate-left" style="color: var(--text-muted);"></i> ${tool.title}
            </a>
          `;
        }
      });
