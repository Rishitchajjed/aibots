...
(function() {
  ...
  window.logSearchQueryToAdmin = function(query) {
    ...
    try {
      // 1. Local history
      const log = JSON.parse(localStorage.getItem('aibots_search_query_log') || '[]');
      if (!log.includes(cleanQ)) {
        log.unshift(cleanQ);
        if (log.length > 50) log.pop();
        localStorage.setItem('aibots_search_query_log', JSON.stringify(log));
      }

      // 2. Push to live Cloud backend
      const cloudApi = localStorage.getItem('aibots_custom_cloud_endpoint') || GOOGLE_DRIVE_CLOUD_API;
      fetch(cloudApi, {
        method: 'POST',
        headers: { 'Content-Type': 'text/plain;charset=utf-8' },
        body: JSON.stringify({ action: 'record_search', query: cleanQ }),
        mode: 'no-cors',
        keepalive: true
      }).catch(() => {});
    } catch(e) {}
